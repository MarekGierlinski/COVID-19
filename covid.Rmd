...
```{r constants}
countries_eu <- c("Italy", "United Kingdom", "France", "Germany", "Netherlands", "Sweden", "Belgium")
#countries <- c(countries_eu, "Japan", "South Korea")
countries <- countries_eu
shapes <- c(15:18, 0:14)
...
```{r plot_function}
basic_plot <- function(d, x="date", xlab="Date", palette=cbPalette, shps=shapes) {
  d %>% 
...
shift_days <- function(d, shifts, day0) {
  d %>% 
    filter(cases > 100) %>% 
    left_join(shifts, by="country") %>% 
    mutate(days = as.integer(date) - shift - day0)
}
```
...
```{r us, fig.width=4, fig.height=3}
d <- covid %>% shift_days(shifts, ref_day0)
d_eu <- d %>% filter(country %in% countries_eu)
d_us <- d %>% filter(country == "United States")
g <- basic_plot(d_eu, x="days", xlab="Normalized day", palette=rep("grey",10), shps=rep(1, 10)) +
  theme(legend.position = "none") +
  geom_point(data=d_us, shape=21, fill="black", colour="black", size=3) +
  geom_line(data=d_us, colour="black")
g

ggsave("fig/us_eu.png", plot=g, device="png", width=5, height=3, dpi=300)
